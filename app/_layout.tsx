...
// 🧠 Prevent splash from hiding before DB setup
...
export default function RootLayout() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        await setupMachineDB();         // ✅ Create SQLite tables if not exist
        await syncLatestMachineData();  // ✅ Fetch & store recent machine data
      } catch (error: any) {
        console.warn('⚠️ Machine DB init error:', error.message);
      } finally {
        // ✅ Hide splash screen once init is done
